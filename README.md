...
Claude Desktop does not accept direct HTTP MCP entries with `type`, `url`, and `headers` in `claude_desktop_config.json`. The plugin generates a Claude Desktop snippet that runs `npx mcp-remote@latest` as a stdio bridge and passes the Authorization header directly in the `--header` argument.

For LocalWP `.local` sites, the Claude Desktop snippet uses `http://` instead of `https://` and includes `--allow-http`. This avoids Node/mcp-remote rejecting LocalWP's self-signed HTTPS certificate with `DEPTH_ZERO_SELF_SIGNED_CERT`, while satisfying mcp-remote's explicit opt-in requirement for non-HTTPS URLs. Direct HTTP snippets for other clients can still use the canonical HTTPS endpoint.
